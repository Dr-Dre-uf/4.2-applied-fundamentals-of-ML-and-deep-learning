...
def display_system_monitor():
    ...
    c1.metric("CPU Usage", f"{cpu_percent}%", help="This will spike when the neural network is actively training.")
    c2.metric("RAM Footprint", f"{mem_mb:.1f} MB", help="Current memory usage of the application and dataset.")
...
track = st.sidebar.radio("Focus Area", ["Clinical Science", "Foundational Science"], 
                         help="Clinical focuses on patient care and hospital utility; Foundational focuses on algorithmic mechanisms.")
st.sidebar.markdown("---")
...
# ==========================================
    ...
    epochs = st.sidebar.slider("Training Epochs", 5, 50, 20, help="Number of complete passes through the training dataset.")
    
    if st.button("Train Base Model", help="Execute the initial training to observe standard accuracy."):
        X = df.iloc[:, :-1].values
        y = df.iloc[:, -1].values
        ...
        st.warning("Critical Thinking Question: Is total accuracy a good evaluation metric for this imbalanced mortality case?")
...
# ==========================================
    ...
    threshold = st.slider("Classification Threshold", 0.1, 0.9, 0.5, 
                          help="Changing this threshold shifts how the model categorizes 'at-risk' patients.")
    
    st.metric("Model Sensitivity", "0.84", delta="Goal: High Recall", help="The ability of the model to correctly identify all patients who died.")
    st.metric("Model Specificity", "0.72", delta="-0.05", delta_color="inverse", help="The ability of the model to correctly identify patients who survived.")
